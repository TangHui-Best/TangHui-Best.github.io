...
| F002.5 | 2026-09-20 | 7bf1779 | 用户指出上线页没有还原已确认的 A 图，首屏和案例文案也显得泛化。 | 实现将预览图当成氛围参考，而未将宽画布、大姓名、深色人物档案和横向案例结构作为必须保留的视觉契约。 | 以 A 图为北极星重做宽画布首屏、人物档案和案例档案行，并将文案改为具体工程位置表达。 | done |
| F002.6 | 2026-09-21 | a353108 | 用户反馈职业照与页面不协调。 | 细长容器配合 `object-fit: cover` 裁切了接近 9:10 的原图，使面部被过度放大。 | 改为按原始比例呈现的档案照，缩小比例、保留四周深色留白，让姓名和工程定位回到首屏主层级。 | done |
| F002.7 | 2026-09-21 | c4ad41a | 用户反馈白底职业照与深色大卡片属于不同视觉语言，仍然不协调。 | 虽已修正裁切，但保留深色海报容器让照片像被拼贴到另一个页面。 | 移除深色卡片，将照片改为与页面同色系的纸质档案照，以细框、分隔线和信息栏建立关联。 | done |
| F002.8 | 2026-09-21 | 54f6f5c | 用户确认 F002.7 的纸质档案照比上一版更丑，要求恢复。 | F002.7 是基于局部视觉判断的主动替换，没有延续用户对上一版深色档案卡的偏好。 | 精确恢复 F002.6 的深色档案卡与完整比例照片；后续不在未确认的情况下更换首屏照片与容器的整体关系。 | done |

## Patch Churn Review

- 轨迹：F002.1 解决一级导航重复，F002.2 补足个人判断与工作外兴趣，F002.3 补足招聘方所需的职业事实；F002.4 以案例档案重组 About Me，F002.5 将 A 图中的宽画布和人物档案结构落实到页面，F002.6 修复真实职业照被过度裁切放大的问题，F002.7 尝试移除深色容器，F002.8 按用户反馈精确恢复 F002.6。
- 根因：前半段缺少一次性确认可公开履历和 70% 专业、20% 思考、10% 生活的层级；后半段把视觉预览当成氛围参考，只验证了构图轮廓。F002.7 又将“照片与背景不协调”的局部问题错误地扩大为替换整体视觉关系，未先验证用户是否更偏好上一版的整体构图。
- 上游保护：`PRODUCT.md` 已固化招聘方优先、以具体工程问题建立可信度的原则；F002 明确公开边界。About Me 后续涉及照片时，必须在桌面与 390px 视口实测原始素材比例和背景协调性；若已经有用户确认的整体构图，只可在该构图内做局部调整，替换照片与容器的整体关系前须先获得用户确认。
- 结论：不需要 ADR 或独立 Lesson。本次以 F002.8 回退记录锁定用户确认的基线，避免继续在同一视觉决策上迭代猜测。
