...
- 轨迹：F002.1 解决一级导航重复，F002.2 补足个人判断与工作外兴趣，F002.3 补足招聘方所需的职业事实；F002.4 以案例档案重组 About Me，F002.5 将 A 图中的宽画布和人物档案结构落实到页面，F002.6 修复真实职业照被过度裁切放大的问题，F002.7 则移除与白底职业照相冲突的深色海报容器。
- 根因：前半段缺少一次性确认可公开履历和 70% 专业、20% 思考、10% 生活的层级；后半段把视觉预览当成氛围参考，只验证了构图轮廓，既没有以真实职业照检查图片比例和视觉权重，也没有验证照片与背景容器是否属于同一视觉系统。
- 上游保护：`PRODUCT.md` 已固化招聘方优先、以具体工程问题建立可信度的原则；F002 明确公开边界。About Me 后续涉及照片时，必须在桌面与 390px 视口实测原始素材比例和背景协调性；白底职业照应作为纸质身份档案而非深色海报主视觉，不使用 `object-fit: cover` 放大裁切，也不置于大面积深色容器中。
- 结论：不需要 ADR 或独立 Lesson。此次保护已经落在 F002 的视觉约束、样式实现和浏览器验证中。
