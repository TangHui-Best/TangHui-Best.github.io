...
位置：放在「这篇文章会建立什么认知框架」的 Mermaid 总图之后

图要回答的问题：一项模糊的代码问题，怎样一步步变成可验证的局部证据？

读者看图后应复述的结论：代码探索不是一次把仓库塞进模型，而是从候选路径逐轮缩小到可回查的源码证据。

图像类型：机制图解插画

画面描述：左到右四段：写着“为什么认证失败？”的问号便签；一排文件夹代表候选路径；放大镜指向带行号的纸张代表 Grep 命中；最后是一小段被框选的源码纸张和一个打勾的测试文件代表局部证据。四段之间以方向明确的箭头连接，底部一条细线标“每一步只获取决定下一步的信息”。

图内标签或后期图例：模糊问题、候选路径、命中行、局部源码、可回查证据。

事实边界：这是对文中机制关系的解释性图解，不呈现特定版本源码或实际工具界面。

避免：不要画真实 Claude 品牌标志、机器人、代码雨、芯片、无意义悬浮窗口或虚构的工具返回字段。

中文生图提示词（可直接复制）：

```text
请画一张中文技术图解插画，横版 16:9，浅灰白背景，高对比度，阅读方向从左到右，箭头清晰。

【这张图要讲什么】它解释 AI 编程助手怎样探索一个陌生代码库。用户一开始只有一个模糊问题，例如“为什么认证失败？”系统不应把整个仓库一次塞进模型，而是分四步收集证据：先找到可能相关的文件路径，再在文件中找到带行号的关键词命中，随后只阅读命中附近的一小段真实代码，最后得到可以回查和验证的证据。观众看完图应能说出：代码探索是逐步缩小范围的证据循环，每一步只获取决定下一步所需的信息。

【视觉翻译】从左到右画四个明显分区。第一格是一张带大问号的便签，代表“模糊问题”；第二格是三四个按文件夹排列的文件夹，代表“候选路径”；第三格是一只放大镜照着一张纸上唯一被高亮的短行，纸边清晰显示行号，代表“命中行”；第四格是一张只展开小范围内容的源码纸和一份带勾的测试纸，代表“局部源码”和“可回查证据”。每格只保留一个关键对象，用粗箭头连接，箭头越来越聚焦。底部放一条细的预算尺，分四段从宽到窄，表达每一步都控制信息量；不要画具体数字或品牌界面。

【文字标签】模糊问题、候选路径、命中行、局部源码、可回查证据，共五个简体中文短语。若中文渲染不可靠，宁可省略文字，由人工后期叠加。

【风格与禁忌】扁平矢量或简洁等距视角，留白充分，配色克制，箭头和对象关系必须一眼可读。禁止出现大脑、机器人、人形角色、电路板、芯片、代码雨、星云、无意义悬浮 UI、服务器机房、真实品牌标志、英文长句和精确源码内容。
```

建议图注：从路径到局部源码，探索过程的价值在于持续缩小不确定性，而不是一次性召回更多材料。

位置：放在「高噪声探索要隔离，但不能把责任也外包」的流程图之后

图要回答的问题：为什么 Explore subagent 能保护主 Agent 的上下文，却不能替代主 Agent 的验收责任？

读者看图后应复述的结论：subagent 保存搜索噪声，只回传结论、证据位置和未确认项；主 Agent 仍负责验证、修改与交付。

图像类型：机制图解插画

画面描述：左右分栏。左边是一张干净的主工作台，摆着目标卡、方案卡、测试清单；右边是带抽屉的研究台，抽屉里装大量搜索纸条和日志，顶上只送出三张小卡片“结论”“证据位置”“未确认项”到主工作台。主台最后盖上“验证后交付”的印章。箭头只从研究台的三张卡片指向主台。

图内标签或后期图例：主 Agent、探索噪声、结论、证据位置、未确认项、验证后交付。

事实边界：主从分工是本文的工程解释；Explore 只读和独立上下文用途来自官方文档，图中具体工作台是比喻。

避免：不要将 subagent 画成真人雇员、不要暗示它自动完成代码修改或自动通过测试，不要出现真实品牌标志。

中文生图提示词（可直接复制）：

```text
请画一张中文技术图解插画，横版 16:9，浅色背景，左右分栏，高对比度，关系明确。

【这张图要讲什么】它解释 AI 编程助手为什么把高噪声代码搜索交给一个只读的探索子助手。右侧子助手可以翻阅大量文件、搜索结果和日志，这些中间材料不进入主助手的工作记忆；它只能把三样压缩材料交回去：结论、证据位置、未确认项。左侧主助手保留任务目标、修改方案和测试清单，最后由主助手验证后交付。观众看完图应能说出：子助手隔离搜索噪声，但不能替主助手承担验证和交付责任。

【视觉翻译】左侧是一张整洁工作台，上面有三张固定在软木板上的卡片，分别代表目标、方案、测试；右侧是一张带多层抽屉的研究台，抽屉中露出很多小纸条和日志纸，代表大量探索材料。研究台上方只出现三张大小一致、整齐排列的小卡片，分别代表结论、证据位置、未确认项；三张卡片沿单向箭头从右送到左。左侧工作台末端有一个清晰的印章压在一份纸上，表示验证后交付。不要把右侧的材料直接倾倒到左侧。

【文字标签】主 Agent、探索噪声、结论、证据位置、未确认项、验证后交付，共六个简体中文短语。若中文渲染不可靠，宁可省略文字，由人工后期叠加。

【风格与禁忌】简洁扁平插画，主工作台留白多、研究台信息密但不凌乱，粗箭头清晰表达单向回传。禁止出现人形角色、机器人、大脑、电路板、芯片、代码雨、霓虹数据流、无意义悬浮 UI、真实品牌标志、英文长句和暗示自动验收通过的绿色仪表盘。
```

建议图注：Explore 的独立上下文保存过程材料，主 Agent 保留最终判断与验收责任。
